...
frames = tuple([f'E:/mirai/data/Petpet/frames/frame{i}.png' for i in range(5)])
...
async def petpet(member_id, dest, flip=False, squish=0, fps=20) -> None:
    """生成PetPet
    ...
    # 输出
    await save_gif(gif_frames, dest, fps=fps)


if __name__ == '__main__':
    tempDir = 'E:/mirai/data/Petpet/temp'
    if not os.path.exists(tempDir):
        os.mkdir(tempDir)
    dest = tempDir + f'/tempPetPet-{sys.argv[1]}.gif'
    loop = asyncio.get_event_loop()
    task = loop.create_task(petpet(sys.argv[1], dest))
